...
IniCheckBox::IniCheckBox(const QString &id, const QString &text, QWidget *buddyWidget) : CustomIniEditor(id, &editor, buddyWidget)
{
	...
	switch (CustomIniEditor::editorIds.value(id))
	{
		...
		{
			addHintText(QStringLiteral("(Never set) Activates \"Activity Level A\" and \"Activity Level B\""));
			break;
		}
		case 16: // Flowey/AF
		{
		...
		{
			addHintText(QStringLiteral("Reached an ending while \"truename\" is 1"));
			break;
		}
		...
		case 62: // reset/reset
		{
			addHintText(QStringLiteral("Set on naming screen after a true reset. Activates \"Activity Level A\" and \"Activity Level B\""));
			break;
		}
		case 63: // reset/s_key
		{
...
IniSpinBox::IniSpinBox(const QString &id, QWidget *buddyWidget) : CustomIniEditor(id, &editor, buddyWidget)
{
	...
	switch (CustomIniEditor::editorIds.value(id))
	{
		case 0: // General/BC
		{
			addHintText(QStringLiteral("For those \"Get an item\" trophies on PlayStation"));
			break;
		}
		...
		case 15: // General/fun
		{
			addHintText(QStringLiteral("Randomly generated number at the start of the game that determines random events. This value is used in tandem with the \"fun\" in the save file"));
			break;
		}
