...
{
    QStringList names;
    for (const QString &name : Project_Credits)
    {
        names.append(QStringLiteral("**%1**").arg(name)); // Just for making the entry bold
    }

    return names.join(QStringLiteral("  \n"));
}

YellowNamesDialog::YellowNamesDialog(QWidget *parent, Qt::WindowFlags flags) :
    QDialog(parent, flags)
{
    setModal(true);
    setMinimumSize(420, 240);
    ...
    // Using shared pointer will allow referencing it inside other functions
    const QSharedPointer<QList<CustomCheckBox *>> widgetList(new QList<CustomCheckBox *> {
